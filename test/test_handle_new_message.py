...
sys.modules['telethon'] = SimpleNamespace(
    TelegramClient=None,
    events=None
)
...
class FakeEvent:
    def __init__(self, message, chat_title=None, chat_id=None):
        ...
        self.chat = SimpleNamespace(title=chat_title) if chat_title is not None else None
...
class TestHandleNewMessage(unittest.TestCase):
    ...
    def test_close_signal_reply_forwards_and_processes(self):
        ...
        with patch.object(ub, "process_signal", new_callable=AsyncMock) as proc_mock, \
             patch.object(ub, "forward_to_archive", new_callable=AsyncMock) as archive_mock, \
            ...
            self.assertEqual(called_args[0], ub.SignalType.CLOSE, "process_signal was not called with CLOSE signal type")
            # Second argument should be the mapped group id
            self.assertEqual(called_args[1], mapped_gid, "process_signal was not called with the expected group id")

            # Ensure forward_to_archive was awaited
            self.assertTrue(archive_mock.await_count >= 1, "forward_to_archive was not awaited")
            archive_args = archive_mock.call_args[0]
            self.assertIs(archive_args[0], fake_message, "forward_to_archive did not receive the correct message")
            self.assertEqual(archive_args[1], chat_title, "forward_to_archive did not receive the correct chat title")
            self.assertEqual(archive_args[2], mapped_gid, "forward_to_archive did not receive the correct group id")
