...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_malformed_signal_text(self):
        ...
        for signal in malformed_signals:
            with self.subTest(signal=signal):
                ...
                self.assertIsNone(result, f"Should return None for malformed signal: {signal}")
    ...
    def test_very_long_channel_name(self):
        ...
        for name in long_names:
            with self.subTest(name=name):
                ...
                self.assertLessEqual(len(cleaned), 4, f"Channel name should be max 4 chars: {cleaned}")
                self.assertGreater(len(cleaned), 0, f"Channel name should not be empty: {cleaned}")
                # Should contain only alphanumeric characters
                self.assertTrue(cleaned.isalnum(), f"Channel name should be alphanumeric: {cleaned}")
    ...
    def test_sl_between_entry_and_tp(self):
        ...
        if result:
            ...
            self.assertLess(result["stop_loss"], result["entry"], "SL should be below entry for BUY")
    ...
    def test_mt4_comment_format_edge_cases(self):
        ...
        for digits in [0, 1, 2, 3, 4, 5, 6]:
            ...
            self.assertLessEqual(len(comment), 31, f"Comment should be <= 31 characters for {digits} digits")
