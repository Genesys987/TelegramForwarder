...
class TestPythonMT4Compatibility(unittest.TestCase):
    ...
    def test_channel_name_python_vs_mt4_logic(self):
        ...
        for channel in test_cases:
            ...
            self.assertEqual(len(python_result), 4, f"Python result should be 4 chars: {python_result}")
            self.assertEqual(len(mt4_result), 4, f"MT4 result should be 4 chars: {mt4_result}")
            self.assertTrue(python_result.isalnum(), f"Python result should be alphanumeric: {python_result}")
            self.assertTrue(mt4_result.isalnum(), f"MT4 result should be alphanumeric: {mt4_result}")
            
            print(f"  '{channel}' -> Python: '{python_result}', MT4 equivalent: '{mt4_result}'")
    ...
    def test_comment_format_python_vs_mt4_parsing(self):
        ...
        for gid, channel, sl, digits in test_cases:
            ...
            self.assertEqual(len(parts), 3, f"Comment should have 3 parts: {python_comment}")
            
            # Parse GID
            parsed_gid = int(parts[0])
            self.assertEqual(parsed_gid, gid, f"GID should match: {parsed_gid} vs {gid}")
            
            # Parse channel
            parsed_channel = parts[1]
            self.assertEqual(len(parsed_channel), 4, f"Channel should be 4 chars: {parsed_channel}")
            
            # Parse SL
            parsed_sl = float(parts[2])
            self.assertAlmostEqual(parsed_sl, sl, places=2, msg=f"SL should match: {parsed_sl} vs {sl}")
            
            print(f"  GID:{gid}, Channel:'{channel}', SL:{sl} -> '{python_comment}' -> Parsed: GID:{parsed_gid}, Channel:'{parsed_channel}', SL:{parsed_sl}")
            ...
            if os.path.exists(queue_path):
                with open(queue_path, 'r', encoding='utf-8') as f:
                    ...
                    if lines:
                        ...
                        self.assertGreaterEqual(len(parts), 8, f"Should have at least 8 parts: {len(parts)}")
        ...
        for comment in old_format_comments:
            ...
            if comment.startswith("GID:"):
                ...
                if sl_part:
                    ...
                    self.assertEqual(len(channel), 4, f"Channel should be 4 chars: {channel}")
    ...
    def test_array_bounds_and_tp_handling(self):
        ...
        for i, tps in enumerate(tp_test_cases):
            ...
            print(f"  {len(tps)} TPs -> Python: ✅, MT4 equivalent: {mt4_tp_count} TPs (max 20)")
            
            if len(tps) > 20:
                print(f"    Warning: MT4 would truncate to 20 TPs")
