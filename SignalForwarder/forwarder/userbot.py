...
try:
    ...
    logging.error(f"Hiba: Hianyzó alap beállítások a config.py-ban: {e}")
    exit()

# --- Feldolgozó és segéd modulok importálása ---
try: from signal_parser import parse_signal
except ImportError: logging.error("Hiba: signal_parser.py/parse_signal hiányzik."); exit()
try: from queue_manager import add_signal_to_queue
except ImportError: logging.error("Hiba: queue_manager.py/add_signal_to_queue hiányzik."); exit()
try: from stoploss_update import process_stoploss_reply
except ImportError: logging.error("Hiba: stoploss_update.py/process_stoploss_reply hiányzik."); exit()
...
def load_last_gid(): # Betöltés indításkor
    ...
    try:
        if os.path.exists(LAST_GID_FILE):
            ...
            logging.info(f"Utolsó GID betöltve: {current_group_id}. Következő: {current_group_id + 1}")
        else: logging.info(f"{LAST_GID_FILE} nem található, GID indul: {current_group_id + 1}")
    except Exception as e: logging.error(f"Hiba GID betöltésekor: {e}. Indul: {current_group_id + 1}")
def save_last_gid(): # Mentés növelés után
    global current_group_id
    try:
        with open(LAST_GID_FILE, "w") as f: f.write(str(current_group_id))
    except Exception as e: logging.error(f"Hiba GID mentésekor: {e}")
...
def load_message_gid_map(): # Betöltés indításkor
    ...
    try:
        if os.path.exists(MESSAGE_GID_MAP_FILE):
            ...
            logging.info(f"{len(message_id_to_group_id)} üzenet->GID map betöltve.")
        else: logging.info(f"{MESSAGE_GID_MAP_FILE} nem található, üres map."); message_id_to_group_id = {}
    except Exception as e: logging.error(f"Hiba map betöltésekor: {e}"); message_id_to_group_id = {}
def save_message_gid_map(): # Mentés hozzáadás után
    ...
    except Exception as e: logging.error(f"Hiba map mentésekor: {e}")
...
async def process_new_standard_signal(message_text: str, message_id: int, message_date, channel_name: str = None):
    logging.info(f"   Standard szignál feldolgozása (ID: {message_id}) csatornából: {channel_name or 'UNKNOWN'}...")
    signal_data = parse_signal(message_text)
    if not signal_data: logging.warning(f"   Szignál parse sikertelen."); return None
    
    # Add cleaned channel name to signal data
    if channel_name:
        clean_name = clean_channel_name(channel_name)
        signal_data["channel_name"] = clean_name
        logging.info(f"   Csatorna név hozzáadva: '{channel_name}' -> '{clean_name}'")
    else:
        signal_data["channel_name"] = clean_channel_name("UNKNOWN")
        logging.warning(f"   Figyelmeztetés: Nincs csatorna név, 'UNKN' használata")
    ...
    if message_date:
        ...
        logging.info(f"   Timestamp hozzáadva: {timestamp} ({message_date.isoformat()})")
    else:
        logging.warning(f"   Figyelmeztetés: Nincs üzenet dátum, jelenlegi időt használjuk")
        timestamp = int(datetime.now(timezone.utc).timestamp())
        signal_data["timestamp_utc"] = timestamp

    group_id = get_next_group_id() # Generáljuk az ÚJ GID-t
    signal_data["group_id"] = group_id # Hozzáadjuk a dict-hez
    logging.info(f"   Új GroupID: {group_id}")

    add_gid_mapping(message_id, group_id) # Eltároljuk az összerendelést
    logging.info(f"   Map tárolva: MsgID {message_id} -> GID {group_id}")

    # Hozzáadjuk a queue-hoz (a queue manager formázza és írja a queue fájlba)
    if add_signal_to_queue(signal_data): logging.info(f"   Jelzés queue-hoz adva (GID {group_id})."); return group_id
    else: logging.error(f"   Hiba: Jelzés queue-hoz adása sikertelen (GID {group_id})."); return None
...
async def run_userbot():
    ...
    try: await client.start(); logging.info("Sikeres csatlakozás Telethon kliensként.")
    except Exception as e: logging.error(f"Hiba kliens indításakor: {e}"); return
    ...
    logging.info("Csatorna cache kiírva 'channels.txt'-be.")
    joined_chats_entity = []
    if not INVITE_LINKS: logging.warning("Figyelmeztetés: Nincsenek csatornák megadva.")
    else:
        for link_or_channel_id in INVITE_LINKS:
            try:
                logging.info(f"Csatlakozás ehhez: {link_or_channel_id}...")
                ...
                logging.info(f"✅ Figyelés beállítva erre: {title}")
                joined_chats_entity.append(entity)
            except Exception as e: logging.error(f"❌ Hiba csatorna kezelésekor ({link_or_channel_id}): {e}")
    if not joined_chats_entity:
        logging.warning("Figyelmeztetés: Nem figyelünk csatornákat.")
        quit()
    ...
    async def new_message_handler(event):
        ...
        logging.info(f"📩 Új üzenet innen: '{chat_title}' (ID: {message_id})")

        # === Reply üzenet kezelése ===
        if message.is_reply:
            reply_to_msg_id = message.reply_to_msg_id
            logging.info(f"   Válasz. Eredeti ID: {reply_to_msg_id}")
            ...
            if reply_original_message:
                ...
                if re.search(r'(adjust|set|move)\s+(your\s+)?sl', message_text, re.IGNORECASE):
                    logging.info(f"   SL állítás detektálva.")
                    # --- === GROUP_ID MEGSZERZÉSE === ---
                    retrieved_group_id = message_id_to_group_id.get(reply_to_msg_id)
                    # --- =========================== ---
                    if retrieved_group_id:
                        logging.info(f"   Talált GID: {retrieved_group_id}")
                        command_written = process_stoploss_reply(message_text, original_message_text, retrieved_group_id)
                        if command_written: logging.info(f"   ✅ SL parancs kiírva.")
                        else: logging.error(f"   ❌ SL parancs hiba.")
                    else: logging.warning(f"   FIGYELEM: Nem található GID (ID: {reply_to_msg_id}). SL válasz nem feldolgozható!")
                else: logging.info(f"   Nem SL állításnak tűnő válasz.")
            else: logging.error(f"   Hiba: Eredeti üzenet lekérése sikertelen (ID: {reply_to_msg_id}).")
            return
        ...
        # === Standard szignál feldolgozás ===
            ...
            try:
                ...
                logging.error(f"   Hiba process_new_standard_signal hívásakor: {e}"); traceback.print_exc()

    logging.info("🟢 Userbot elindult. Várakozás...")
