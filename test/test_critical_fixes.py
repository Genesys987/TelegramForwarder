...
class TestCriticalFixes(unittest.TestCase):
    ...
    def test_channel_name_uniqueness(self):
        ...
        test_cases = [
            ...
            (
                "PREMIUM_SIGNALS_PRO",
                "PREA",
            ),  # Note: might be same due to hash collision
        ...
        for original, expected in test_cases:
            ...
            self.assertEqual(
                len(result), 4, f"Channel name should be 4 chars: {result}"
            )
            self.assertTrue(
                result.isalnum(), f"Channel name should be alphanumeric: {result}"
            )

        # Check for better uniqueness (allowing some collisions due to hash)
        unique_codes = set(results.values())
        print("Channel name mapping results:")
    ...
    def test_comment_format_length_limit(self):
        ...
        for gid, channel, sl, digits in test_cases:
            ...
            self.assertLessEqual(
                len(comment),
                31,
                f"Comment too long: '{comment}' ({len(comment)} chars)",
            )
    ...
    def test_signal_validation_robustness(self):
        ...
        many_tp_signal = SignalData(
            **{
                **base_signal.__dict__,
                "take_profits": [1.1250 + i * 0.001 for i in range(15)],
            }
        )
