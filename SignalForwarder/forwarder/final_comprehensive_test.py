...
SL 1.1345""",
        ...
        print(f"✅ All comment formats within 31 character limit")
        print(f"✅ All channel names cleaned to 4 characters")
        print(f"✅ All signals added to queue successfully")
        ...
        for queue_path in MT4_QUEUE_FILE_PATHS:
            self.assertTrue(os.path.exists(queue_path), f"Queue file missing: {queue_path}")
            with open(queue_path, 'r', encoding='utf-8') as f:
                content = f.read()
                self.assertGreater(len(content), 0, f"Queue file empty: {queue_path}")
                lines = [line for line in content.strip().split('\n') if line.strip()]
                self.assertEqual(len(lines), len(test_signals), f"Wrong number of signals in queue: {len(lines)} != {len(test_signals)}")
        ...
        for signal_path in MT4_SIGNAL_FILE_PATHS:
            self.assertTrue(os.path.exists(signal_path), f"Signal file not created: {signal_path}")
        ...
        self.assertLessEqual(len(long_comment), 31, f"Long comment too long: {long_comment}")
        print(f"  ✅ Long values handled: '{long_comment}' ({len(long_comment)} chars)")
        
        # Empty channel name
        empty_channel = clean_channel_name("")
        self.assertEqual(empty_channel, "UNKN", f"Empty channel not handled: {empty_channel}")
        print(f"  ✅ Empty channel handled: '{empty_channel}'")
        
        # Non-alphabetic channel name
        numeric_channel = clean_channel_name("123456")
        self.assertEqual(numeric_channel, "UNKN", f"Numeric channel not handled: {numeric_channel}")
        ...
        self.assertEqual(len(short_channel), 4, f"Long channel not truncated: {short_channel}")
        print(f"  ✅ Long channel truncated: '{long_channel_name}' -> '{short_channel}'")
